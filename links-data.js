window.siteLinks = {
  social: [
    { label: { de: 'YouTube', en: 'YouTube' }, url: 'https://www.youtube.com/@JuliusDomroes', icon: 'fa-youtube' },
    { label: { de: 'Instagram', en: 'Instagram' }, url: 'https://www.instagram.com/juliusdomroes/', icon: 'fa-instagram' },
    { label: { de: 'TikTok', en: 'TikTok' }, url: 'https://www.tiktok.com/@juliusdomroes', icon: 'fa-tiktok' }
  ],
  affiliates: [
    {
      title: { de: 'Superteam Wheels', en: 'Superteam Wheels' },
      description: { de: '5% Rabatt auf S-All Carbon Ultra H2', en: '5% off the S-All Carbon Ultra H2' },
      url: 'https://superteamwheels.com/discount/JULIUS5?ref=JULIUS',
      ctaLabel: { de: 'Zum Angebot (Werbung)', en: 'Shop now (Ad)' },
      copyValue: 'JULIUS5'
    },
    {
      title: { de: '10plus Wheels', en: '10plus Wheels' },
      description: { de: '5% Rabatt auf die T Series (Road)', en: '5% off the T Series for Road' },
      url: 'https://10plus.cool/discount/JULIUS5?ref=JULIUS',
      ctaLabel: { de: 'Zum Angebot (Werbung)', en: 'Shop now (Ad)' },
      copyValue: 'JULIUS5',
      extraActions: [
        {
          label: { de: 'Auf AliExpress kaufen (Werbung)', en: 'Shop on AliExpress (Ad)' },
          url: 'https://s.click.aliexpress.com/e/_c3oDWl2T'
        }
      ]
    }
  ]
};
